...
/**
 * IDs of chat bar buttons currently packed away into the Backpack plugin's popout.
 * Packed buttons are hidden from the main chat bar and rendered inside the Backpack popout instead.
 */
export const BackpackedButtons = new Set<string>();
export const backpackListeners = new Set<() => void>();
export function notifyBackpackChange() {
    for (const fn of backpackListeners) {
        try { fn(); } catch (e) { logger.error("Backpack listener threw", e); }
    }
}
