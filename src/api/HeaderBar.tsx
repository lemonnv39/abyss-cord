...
// Shared hide-all-buttons state, used by CompactMode/StealthMode. Buttons
// registered with alwaysVisible stay shown even while hidden (e.g. the
// toggle button itself).
const alwaysVisibleIds = new Set<string>();
let headerBarButtonsHidden = false;

export function isHeaderBarButtonsHidden(): boolean {
    return headerBarButtonsHidden;
}

export function setHeaderBarButtonsHidden(hidden: boolean) {
    headerBarButtonsHidden = hidden;
    headerBarListeners.forEach(listener => listener());
}

/** Marks a previously-added button as always visible, even while buttons are hidden. */
export function setHeaderBarButtonAlwaysVisible(id: string, alwaysVisible: boolean) {
    if (alwaysVisible) alwaysVisibleIds.add(id);
    else alwaysVisibleIds.delete(id);
    headerBarListeners.forEach(listener => listener());
}
...
function HeaderBarButtons() {
    ...
    return Array.from(headerBarButtons)
        .filter(([id]) => !headerBarButtonsHidden || alwaysVisibleIds.has(id))
