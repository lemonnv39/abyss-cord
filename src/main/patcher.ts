...
if (!IS_VANILLA) {
    ...
    //
    // These three switches keep the renderer running at full speed even
    // while minimized/unfocused (no Chromium throttling at all), which is
    // what keeps calls/notifications alive in the background — but it also
    // means Abyss never gets to idle down. Settings > Abyss > "Reduce
    // background CPU usage" opts back into normal Chromium throttling for
    // users who'd rather trade that reliability for lower idle CPU/RAM.
    ...
    // Performance Mode / Max Performance (Settings > Abyss). GPU command-line
    // switches only take effect if set before Electron's GPU process starts,
    // which is why this lives here rather than in a plugin — by the time any
    // plugin's start() runs, it is already too late to change these.
