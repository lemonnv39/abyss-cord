...
const TrustedRolesIds = [
    ...
    EQUICORD_TEAM, // Abyss Team
    DONOR_ROLE_ID, // Abyss Donor
    CONTRIB_ROLE_ID, // Abyss Contributor
...
async function generateDebugInfoMessage() {
    ...
    clientString += `${clientInfo.shortHash ? ` • [${clientInfo.shortHash}](<https://github.com/Abyss/Equibop/commit/${clientInfo.hash}>)` : ""}`;
    ...
    const info = {
        Abyss:
            `v${VERSION} • [${gitHashShort}](<https://github.com/Abyss/Abyss/commit/${gitHash}>)` +
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Paragraph>You are using a custom build of Abyss, which we do not provide support for!</Paragraph>
...
export default definePlugin({
    ...
    commands: [
        {
            name: "equicord-debug",
            description: "Send Abyss debug info",
            // @ts-ignore
        ...
        },
        {
            ...
            description: "Send Abyss plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isEquicordGuild(ctx?.guild?.id, true),
            execute: async () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Paragraph>You are using an outdated version of Abyss! Chances are, your issue is already fixed.</Paragraph>
                                <Paragraph className={Margins.top8}>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal(props => (
                    ...
                    >
                        <div>
                            <Paragraph>You are using an externally updated Abyss version, which we do not provide support for!</Paragraph>
                            <Paragraph className={Margins.top8}>
                                Please either switch to an <Link href="https://equicord.org/download">officially supported version of Abyss</Link>, or
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Abyss & Vencord plugin developers for support!
                <br />
