...
import { BackpackedButtons, backpackListeners, ChatBarButton, ChatBarButtonFactory, ChatBarButtonMap, ChatBarProps, notifyBackpackChange } from "@api/ChatButtons";
                ...
                <Tooltip text={id} key={id}>
                    {(tooltipProps: any) => (
                        <div
                            ...
                            onClick={e => {
                                // Do not auto-close so multiple packed plugins can be used in sequence
                            }}
                        >
                            <ErrorBoundary noop>
                                <data.render
...
const BackpackChatBarButton: ChatBarButtonFactory = props => {
    ...
    // Counts modals/popups currently open ABOVE the backpack
    const overlayCount = useRef(0);

    // Observe everything that appears in Discord's DOM layer
    useEffect(() => {
        if (!isOpen) {
        ...
        // Snapshot of body's children when the backpack opens
        ...
        // Observe direct children of document.body (where Discord inserts its portals)
    ...
    return (
        <Popout
            ...
            shouldShow={isOpen}
            onRequestClose={() => {
                // Do not close if a popup/modal was opened by a backpack plugin
