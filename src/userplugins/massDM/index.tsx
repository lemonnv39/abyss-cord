...
async function startSending(message: string, excludedIds: Set<string> = new Set()) {
    ...
    const friends = getFriendIDs().filter(id => !excludedIds.has(id));
    state.reset();
    ...
    for (const id of friends) {
        ...
        try {
            ...
            if (!dmRes?.body?.id) {
                state.log.push(`Failed: ${name} — channel not found`);
                state.notify();
                continue;
            }
            await RestAPI.post({ url: `/channels/${dmRes.body.id}/messages`, body: { content: personalizedMessage, tts: false } });
            state.done++;
            state.log.push(`Sent: ${name}`);
        } catch (e: any) {
            state.log.push(`Failed: ${name} — ${e?.message ?? "error"}`);
        }
        state.notify();
...
function MassDMModal({ rootProps }: { rootProps: any; }) {
    ...
    const handleMsgChange = (val: string) => {
        setMsg(val);
        setShowMentionHint(val.endsWith("@"));
    };
                                ...
                                renderOptionLabel={(o: any) => {
                                    ...
                                    return (
                                        <div style={{ display: "flex", alignItems: "center", gap: 8, width: "100%" }}>
                                            ...
                                            <span style={{ flex: 1, color: isExcluded ? "var(--status-danger)" : "var(--text-normal)" }}>{o.label}</span>
                                        </div>
                                    );
                                }}
                                renderOptionValue={() => (
            ...
            <ModalContent>
                            ...
                            <div style={{ position: "relative" }}>
                                ...
                                {showMentionHint && (
                                    <div className="mdm-mention-hint" onClick={insertMention}>
                                        <div className="mdm-mention-hint-item"><strong>@user</strong> — mention the recipient</div>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                {idle && (
                    <p className={`mdm-warn ${Margins.top16}`}>
                        ...
                        {editingDelay ? (
                            ...
                            <span className="mdm-delay-value" onClick={() => { setDelayInput(String(s.delayMs / 1000)); setEditingDelay(true); }} title="Click to modify delay">
                                {s.delayMs / 1000}s
                            </span>
                        )}
                        {" "}between each.
                ...
                {(s.running || s.finished) && (
                    <>
                        ...
                        <div className="mdm-bar-bg"><div className="mdm-bar-fill" style={{ width: `${pct}%` }} /></div>
                        {s.finished && <p className="mdm-done">Finished — {s.done} message(s) sent.</p>}
...
function MassDMButton() {
    return (
        <HeaderBarButton icon={MassDMIcon} tooltip="Mass DM" onClick={() => openModal(props => <MassDMModal rootProps={props} />)} />
    );
}
...
export default definePlugin({
    ...
    description: "Sends a message to all your friends with an anti-rate-limit delay. Note: mass-messaging can violate Discord's ToS if used to spam — use with care.",
