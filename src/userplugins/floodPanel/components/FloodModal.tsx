...
import { RestAPI, Select, TextArea, useEffect, useMemo, useRef, useState } from "@webpack/common";
            ...
            <ModalContent className="vc-flood-modal-content">
                ...
                <div className={Margins.bottom16}>
                    <Select
                        ...
                        isSelected={(v: string) => v === delayMs}
                        select={(v: string) => setDelayMs(v)}
                        serialize={(v: string) => v}
                        renderOptionLabel={(o: any) => <div>{o.label}</div>}
                        renderOptionValue={(selected: any[]) => {
                            const option = selected[0];
                            return option ? option.label : delayOptions.find(o => o.value === delayMs)?.label ?? delayMs;
                        }}
                        popoutPosition="top"
                    />
                </div>
