...
import { ChannelStore, Forms, GuildStore, Menu, React, RelationshipStore, RestAPI, showToast, TextInput, Toasts, UserStore } from "@webpack/common";

// Bulk-deletes only messages YOU authored (author.id === your id, checked in
// canDeleteMessage below), across DMs, servers, or specific channel IDs — via
// Discord's own official message-search and delete REST endpoints. Nothing
// here reads or touches any other user's messages or account.
...
let deleteDelay = 800;
try {
    const saved = localStorage.getItem("abyss_mc_delay");
    if (saved) deleteDelay = Math.max(50, Math.min(5000, parseInt(saved, 10) || 800));
} catch { }

let useSilentDelete = false;
try {
    const saved = localStorage.getItem("abyss_mc_silent_delete");
...
export function setDeleteDelay(val: number) {
    ...
    try { localStorage.setItem("abyss_mc_delay", deleteDelay.toString()); } catch { }
}
...
export function setUseSilentDelete(val: boolean) {
    useSilentDelete = val;
    try { localStorage.setItem("abyss_mc_silent_delete", val.toString()); } catch { }
}
...
// Call-notification messages (type 3) can't always be removed with a plain
// DELETE, so this posts a throwaway message to get a fresh delete context,
// then removes both it and the actual target.
    ...
    while (keepRunning && !shouldStop) {
        try {
            ...
            for (const message of validMessages) {
                ...
                await new Promise(resolve => setTimeout(resolve, getDeleteDelay()));
            }
...
.mcv2-title { font-size: 18px; font-weight: 700; color: #f2f3f5; }
.mcv2-subtitle { font-size: 13px; color: #b5bac1; }
.mcv2-tabs {
    display: flex;
    gap: 2px;
    padding: 0 20px;
...
.mcv2-tab:hover { color: #dbdee1; background: rgba(255,255,255,0.02); }
.mcv2-tab.active { color: #f2f3f5; border-bottom-color: #ed4245; }
.mcv2-content { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; }
.mcv2-list-item {
    display: flex;
    align-items: center;
    padding: 10px 14px;
...
.mcv2-list-item:hover { background: #3f4147; }
.mcv2-list-item.selected { border-color: #ed4245; background: rgba(237, 66, 69, 0.05); }
.mcv2-avatar { width: 32px; height: 32px; border-radius: 50%; margin-right: 12px; }
.mcv2-item-name { flex: 1; font-weight: 600; font-size: 14px; color: #dbdee1; }
.mcv2-btn-row { display: flex; gap: 10px; margin-top: 15px; }
.mcv2-primary-btn {
    background: #ed4245; color: white; padding: 10px 20px; border-radius: 8px;
    font-weight: 600; cursor: pointer; text-align: center; flex: 1; border: none;
    transition: background 0.2s;
}
.mcv2-primary-btn:hover { background: #c9383b; }
.mcv2-progress-card { background: #232428; border-radius: 8px; padding: 15px; margin-top: 20px; border: 1px solid #1e1f22; }
.mcv2-progress-bar-wrap { height: 6px; background: #1e1f22; border-radius: 3px; margin-top: 10px; overflow: hidden; }
.mcv2-progress-bar-fill { height: 100%; background: #ed4245; transition: width 0.3s; }
.mcv2-log-item { padding: 12px; background: #2b2d31; border-radius: 8px; margin-bottom: 8px; border-left: 4px solid #ed4245; }
.mcv2-scroller::-webkit-scrollbar { width: 6px; }
.mcv2-scroller::-webkit-scrollbar-track { background: transparent; }
.mcv2-scroller::-webkit-scrollbar-thumb { background: #1a1b1e; border-radius: 3px; }
.mcv2-select-all-btn {
    display: flex; align-items: center; gap: 8px; height: 40px; padding: 0 14px;
    border-radius: 8px; background: #2b2d31; color: #949ba4;
    border: 1px solid rgba(255, 255, 255, 0.08); cursor: pointer; font-size: 13px;
    font-weight: 600; transition: all 0.2s; user-select: none; white-space: nowrap;
}
.mcv2-select-all-btn:hover { background: #383a40; color: #dbdee1; }
.mcv2-select-all-btn.active { background: rgba(237, 66, 69, 0.15); color: #ffffff; border-color: #ed4245; }
.mcv2-select-all-dot { width: 8px; height: 8px; border-radius: 50%; background: #80848e; transition: all 0.2s; }
.mcv2-select-all-btn.active .mcv2-select-all-dot { background: #ed4245; box-shadow: 0 0 6px rgba(237, 66, 69, 0.8); }
.mcv2-setting-card { background: #2b2d31; border-radius: 8px; padding: 18px; border: 1px solid rgba(255, 255, 255, 0.06); }
.mcv2-speed-slider { width: 100%; height: 6px; border-radius: 3px; background: #1e1f22; outline: none; cursor: pointer; accent-color: #ed4245; margin: 12px 0; }
.mcv2-switch-btn {
    position: relative; width: 44px; height: 24px; border-radius: 12px; background: #4e5058;
    border: none; cursor: pointer; transition: background 0.2s ease; padding: 2px; flex-shrink: 0;
}
.mcv2-switch-btn.active { background: #23a55a; }
.mcv2-switch-knob { display: block; width: 20px; height: 20px; border-radius: 50%; background: #ffffff; transition: transform 0.2s ease; transform: translateX(0); }
.mcv2-switch-btn.active .mcv2-switch-knob { transform: translateX(20px); }
`;
...
function MessageCleanerTab() {
    ...
    const channels = React.useMemo(() => {
        ...
        const friendMockChannels = friendIds.map((id: string) => {
            ...
            return { id: channelId, type: 1, recipients: [id] };
        });
        ...
        return allChannels.map((c: any) => {
            ...
            return { id: c.id, name: name || "Unknown DM", icon, rawUser };
        }).filter((c: any) => {
    ...
    const toggleSelectAll = () => {
        ...
        else setSelected(channels.map((c: any) => c.id));
    };
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px", flex: "0 0 auto", marginBottom: "15px" }}>
                <div style={{ flex: 1 }}>
                    <TextInput placeholder="Search DMs..." value={search} onChange={setSearch} />
                </div>
                <button className={`mcv2-select-all-btn ${allSelected ? "active" : ""}`} onClick={toggleSelectAll} type="button" title={allSelected ? "Deselect All" : "Select All"}>
                    <span className="mcv2-select-all-dot" />
                    {allSelected ? "Deselect All" : "Select All"}
                </button>
            </div>
            <div className="mcv2-scroller" style={{ flex: 1, overflowY: "auto", paddingRight: "8px" }}>
                {channels.length === 0 ? <div style={{ textAlign: "center", color: "#b5bac1", padding: "20px" }}>No DMs or Friends found.</div> : channels.map((ch: any) => (
            ...
            <div className="mcv2-btn-row">
                <button className="mcv2-primary-btn" onClick={handleStart} disabled={selected.length === 0 || isQueueRunning} style={{ opacity: selected.length === 0 || isQueueRunning ? 0.5 : 1 }}>
                    Queue Deletion ({selected.length})
                </button>
            </div>
            <ProgressBanner />
        </div>
...
function ServersCleanerTab() {
    ...
    const toggleSelectAll = () => {
        if (allSelected) setSelected([]);
        else setSelected(guilds.map((g: any) => g.id));
    };
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
            <div style={{ marginBottom: "20px" }}>
                <Forms.FormTitle>Channel IDs (comma separated)</Forms.FormTitle>
                <TextInput placeholder="e.g. 123456789012345678, 987654321098765432" value={input} onChange={setInput} />
            </div>
...
function SettingsTab() {
    ...
    const getSpeedLabel = (ms: number) => {
        if (ms <= 300) return `${ms}ms — Ultra Fast (Caution: high rate limits)`;
        if (ms <= 600) return `${ms}ms — Fast`;
        if (ms <= 1200) return `${ms}ms — Balanced / Recommended`;
        return `${ms}ms — Safe / Stealth (Low rate limit risk)`;
    };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px", height: "100%" }}>
            <div className="mcv2-setting-card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "16px" }}>
                    <div>
                        <div style={{ fontSize: "15px", fontWeight: 600, color: "#ffffff" }}>Deletion Speed & Delay</div>
                        <div style={{ fontSize: "13px", color: "#949ba4", marginTop: "4px" }}>
                            Configure the time interval between each deleted message. Lower values delete faster.
                        </div>
                    </div>
                ...
                <input type="range" min={50} max={2500} step={50} value={delay} onChange={e => handleDelayChange(Number(e.target.value))} className="mcv2-speed-slider" />
            ...
            <div className="mcv2-setting-card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ paddingRight: "16px" }}>
                        <div style={{ fontSize: "15px", fontWeight: 600, color: "#ffffff" }}>Silent Delete Mode</div>
                        <div style={{ fontSize: "13px", color: "#949ba4", marginTop: "4px" }}>
                            When enabled, every deletion goes through a throwaway-message step first (needed for call messages regardless). When disabled, regular messages are deleted directly and only calls you started use that step.
                        </div>
                    </div>
                    <button className={`mcv2-switch-btn ${silent ? "active" : ""}`} onClick={() => handleSilentToggle(!silent)} type="button" title={silent ? "Enabled" : "Disabled"}>
...
function LogsTab() {
    return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
            ...
            <div className="mcv2-btn-row">
                <button className="mcv2-primary-btn" onClick={exportLogs} style={{ background: "#4e5058" }}>Export Logs</button>
            </div>
        </div>
    );
}
...
function CleanerModal({ rootProps }: { rootProps: any; }) {
    ...
    const TABS = [
        { id: "dms", label: "Message Cleaner" },
        { id: "servers", label: "Servers Cleaner" },
        { id: "channels", label: "Channels Cleaner" },
    ...
    return (
        <ModalRoot {...rootProps} size="large" className="mcv2-root">
            ...
            <div className="mcv2-header">
                <div style={{ display: "flex", alignItems: "center", gap: "12px", width: "100%" }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: "12px", color: "#b5bac1", marginTop: "2px" }}>Bulk-delete messages you sent, across DMs, servers, and channels.</div>
                    </div>
            ...
            <div className="mcv2-tabs">
                {TABS.map(tab => (
                    <div key={tab.id} className={`mcv2-tab ${activeTab === tab.id ? "active" : ""}`} onClick={() => setActiveTab(tab.id)}>
                        {tab.label}
                    </div>
                ))}
            </div>
...
function HeaderButton() {
    return (
        <HeaderBarButton icon={IconWithProgress} tooltip="Message Cleaner" onClick={() => openModal(props => <CleanerModal rootProps={props} />)} />
    );
}
...
function handleContextClean(type: "channel" | "server" | "dm", targetId: string, name: string) {
    ...
    showToast("Added to Message Cleaner Queue", Toasts.Type.SUCCESS);
}
...
const ChannelContextMenuPatch: NavContextMenuPatchCallback = (children, ctx: { channel?: any; message?: any; } = {}) => {
    ...
    if (isQueueRunning) {
        menuItems.push(<Menu.MenuItem key="stop-cleaning" id="abyss-stop-cleaning" label="Stop Cleaning" color="danger" action={stopQueue} />);
    } else {
        menuItems.push(<Menu.MenuItem key="clean-messages" id="abyss-clean-messages" label="Clean messages" color="danger" action={() => handleContextClean(isPrivateDM ? "dm" : "channel", channel.id, channel.name || "Channel")} />);
    }

    const topGroup = (
        <Menu.MenuGroup key="abyss-cleaner-top-group">
            {menuItems}
...
const UserContextMenuPatch: NavContextMenuPatchCallback = (children, ctx: { channel?: any; message?: any; user?: any; } = {}) => {
    ...
    if (isQueueRunning) {
        menuItems.push(<Menu.MenuItem key="stop-cleaning-user" id="abyss-stop-cleaning-user" label="Stop Cleaning" color="danger" action={stopQueue} />);
    } else {
        menuItems.push(<Menu.MenuItem key="clean-messages-user" id="abyss-clean-messages-user" label="Clean messages" color="danger" action={() => handleContextClean(isPrivateDM ? "dm" : "channel", channel.id, channel.name || user?.username || "User")} />);
    }

    if (menuItems.length === 0) return;

    const topGroup = (
        <Menu.MenuGroup key="abyss-user-cleaner-top-group">
            {menuItems}
...
const GuildContextMenuPatch: NavContextMenuPatchCallback = (children, ctx: { guild?: any; } = {}) => {
    ...
    if (isQueueRunning) {
        menuItems.push(<Menu.MenuItem key="stop-cleaning-guild" id="abyss-stop-cleaning-guild" label="Stop Cleaning" color="danger" action={stopQueue} />);
    } else {
        menuItems.push(<Menu.MenuItem key="clean-guild-messages" id="abyss-clean-guild-messages" label="Clean all my messages" color="danger" action={() => handleContextClean("server", guild.id, guild.name || "Server")} />);
    }

    const topGroup = (
        <Menu.MenuGroup key="abyss-guild-cleaner-top-group">
            {menuItems}
...
const MessageContextMenuPatch: NavContextMenuPatchCallback = (children, ctx: { channel?: any; message?: any; } = {}) => {
    ...
    if (isQueueRunning) {
        menuItems.push(<Menu.MenuItem key="stop-cleaning-msg" id="abyss-stop-cleaning-msg" label="Stop Cleaning" color="danger" action={stopQueue} />);
    } else {
        menuItems.push(<Menu.MenuItem key="clean-messages-msg" id="abyss-clean-messages-msg" label="Clean messages" color="danger" action={() => handleContextClean(isPrivateDM ? "dm" : "channel", channel.id, channel.name || "Channel")} />);
    }

    const topGroup = (
        <Menu.MenuGroup key="abyss-msg-cleaner-top-group">
            {menuItems}
...
export default definePlugin({
    name: "MessageCleaner",
    description: "An advanced UI for bulk-deleting your own messages across DMs, Servers, and Channels. Includes context menu options.",
