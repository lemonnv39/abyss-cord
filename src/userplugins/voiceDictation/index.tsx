...
async function transcribe(blob: Blob): Promise<string> {
    ...
    if (!apiKey) throw new Error("API key missing — set it in VoiceDictation's settings");
...
const VoiceDictationButton: ChatBarButtonFactory = ({ isMainChat }) => {
    ...
    async function startDictation() {
        ...
        const apiKey = settings.store.groqApiKey?.trim();
        if (!apiKey) {
            showToast("Set a Groq API key in VoiceDictation's settings first.", Toasts.Type.FAILURE);
            return;
        }
    ...
    return (
        <ChatBarButton
            ...
            onContextMenu={e => {
                e.preventDefault();
                openPluginModal(plugins["VoiceDictation"]);
            }}
        >
...
export default definePlugin({
    ...
    description: "Real-time voice dictation via Groq Whisper (free). Requires a Groq API key set in this plugin's settings.",
