...
const settings = definePluginSettings({
    deleteOriginalMessage: {
        ...
        description: "Delete the original server-side message after silent edit. If disabled, the original message reappears after a client reload.",
        default: true
    },
    deleteDelay: {
    ...
    suppressNotifications: {
        type: OptionType.BOOLEAN,
        description: "Suppress notifications on the replacement message (recommended for DMs)",
        default: false
    }
});
...
function sendMessage(content: string, nonce: string, channelId: string, suppressNotifications: boolean, messageReference?: any) {
    const body: any = { content, flags: suppressNotifications ? 4096 : 0, mobile_network_type: "unknown", nonce, tts: false };
    if (messageReference) {
        body.message_reference = { channel_id: messageReference.channel_id, message_id: messageReference.message_id, guild_id: messageReference.guild_id };
    }
    return RestAPI.post({ url: Constants.Endpoints.MESSAGES(channelId), body });
}

function deleteMessage(channelId: string, messageId: string) {
    return RestAPI.del({ url: Constants.Endpoints.MESSAGE(channelId, messageId) });
}
...
const triggerSilentEdit = async (msg: any) => {
    ...
    MessageActions.editMessage = async function (channelId: string, messageId: string, content: any) {
        ...
        if (messageId !== msg.id) return originalEditMessage.apply(this, arguments as any);
...
const messageContextMenuPatch: NavContextMenuPatchCallback = (children, { message }) => {
    ...
    group.push(
        <Menu.MenuItem
            ...
            icon={SilentEditIcon}
            iconLeft={SilentEditIcon}
            leadingAccessory={{ type: "icon", icon: SilentEditIcon }}
        />
    );
};
...
export default definePlugin({
    ...
    description: "\"Silently\" edits a message without the edit tag, bypassing message loggers that watch for it. Only affects your own messages.",
