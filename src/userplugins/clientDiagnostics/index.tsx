...
import definePlugin, { OptionType, type Plugin } from "@utils/types";
...
interface PerformanceWithMemory extends Performance { memory?: PerformanceMemory; }
interface ActiveContext { pluginName: string; surface: string; }
interface SurfaceStats { calls: number; totalMs: number; maxMs: number; slowCalls: number; asyncMs: number; }
interface PluginStats {
    name: string; calls: number; totalMs: number; maxMs: number; slowCalls: number; asyncCalls: number; asyncMs: number;
    errors: number; heapIncrease: number; heapDecrease: number; lastHeapDelta: number; intervals: number;
    pendingTimeouts: number; animationFrames: number; listeners: number; firstSeen: number; lastSeen: number;
    surfaces: Record<string, SurfaceStats>;
}
interface DiagnosticsRow extends PluginStats { enabled: boolean; api: boolean; impact: number; hotSurface: string; resources: number; }
interface SourceSnippet { surface: string; label: string; code: string; }
interface ImpactReason { label: string; value: string; description: string; level: "high" | "medium" | "low"; }
interface ImpactAnalysisItem { row: DiagnosticsRow; reasons: ImpactReason[]; recommendation: string; shouldDisable: boolean; summary: string; snippets: SourceSnippet[]; }

const PLUGIN_NAME = "ClientDiagnostics";
const ENTRY_KEY = "abyss_client_diagnostics";
...
const cl = (...names: string[]) => names.map(n => `vc-client-diagnostics-${n}`).join(" ");
const logger = new Logger("ClientDiagnostics");

const GUIDE_ITEMS = [
    { label: "Impact", description: "An internal score combining CPU time, slow spikes, slow calls, async time, heap growth, and active resources. Higher means it deserves more attention." },
    { label: "CPU", description: "Total time measured plugin functions spent on the JavaScript thread since collection started or was reset." },
    { label: "Calls", description: "How many times a measured plugin function was called." },
    { label: "Slow", description: "How many calls passed the slow-call threshold configured in settings." },
    { label: "Heap +", description: "Extra JavaScript memory observed during plugin calls (Chromium heap data, not full process RAM)." },
    { label: "Resources", description: "Still-active resources created by the plugin — intervals, pending timeouts, animation frames, listeners." },
    { label: "Listeners", description: "Event listeners registered while the plugin was running measured code." },
    { label: "Hot surface", description: "The plugin surface that consumed the most total time (start, flux, interval, command, context menu, etc.)." },
    { label: "Max", description: "The slowest single call observed for that plugin." }
];

const MONITOR_GUIDE_ITEMS = [
    { label: "Extra CPU", description: "Compares the plugin's CPU time with the time elapsed since reset — the extra observed work." },
    { label: "CPU share", description: "How much of the measured plugin CPU time belongs to the selected plugin." },
    { label: "Extra RAM", description: "Compares the plugin's observed heap growth with the JavaScript heap currently in use." },
    { label: "RAM share", description: "How much of the measured heap growth belongs to the selected plugin." }
];

const settings = definePluginSettings({
    sortBy: {
        type: OptionType.SELECT,
        description: "Default sorting in the diagnostics table.",
        options: [
            { label: "Impact", value: "impact", default: true }, { label: "CPU time", value: "cpu" },
            { label: "Memory delta", value: "memory" }, { label: "Calls", value: "calls" }, { label: "Resources", value: "resources" }
        ]
    },
    showDisabled: { type: OptionType.BOOLEAN, description: "Include disabled plugins in the table.", default: false },
    showApiPlugins: { type: OptionType.BOOLEAN, description: "Include internal API plugins in the table.", default: false },
    refreshMs: { type: OptionType.SLIDER, description: "How often the diagnostics page refreshes.", markers: [500, 1000, 2000, 5000], default: 1000 },
    slowCallThresholdMs: { type: OptionType.SLIDER, description: "Callback time that counts as a slow call.", markers: [8, 16, 33, 50, 100], default: 16 },
    trackMemory: { type: OptionType.BOOLEAN, description: "Measure Chromium JS heap deltas when available.", default: true },
});
...
function getPluginStats(pluginName: string) {
    ...
    if (!stat) {
        const now = Date.now();
        stat = { name: pluginName, calls: 0, totalMs: 0, maxMs: 0, slowCalls: 0, asyncCalls: 0, asyncMs: 0, errors: 0,
            heapIncrease: 0, heapDecrease: 0, lastHeapDelta: 0, intervals: 0, pendingTimeouts: 0, animationFrames: 0,
            listeners: 0, firstSeen: now, lastSeen: now, surfaces: {} };
        stats.set(pluginName, stat);
    }
    return stat;
}
function getSurfaceStats(stat: PluginStats, surface: string) {
    return stat.surfaces[surface] ??= { calls: 0, totalMs: 0, maxMs: 0, slowCalls: 0, asyncMs: 0 };
}
...
function recordCall(pluginName: string, surface: string, duration: number, failed: boolean, beforeHeap?: number, afterHeap?: number) {
    ...
    stat.calls++; stat.totalMs += duration; stat.maxMs = Math.max(stat.maxMs, duration); stat.lastSeen = Date.now();
    surfaceStat.calls++; surfaceStat.totalMs += duration; surfaceStat.maxMs = Math.max(surfaceStat.maxMs, duration);
    if (isSlow) { stat.slowCalls++; surfaceStat.slowCalls++; }
    if (failed) stat.errors++;
    if (beforeHeap !== undefined && afterHeap !== undefined) {
        const delta = afterHeap - beforeHeap;
        stat.lastHeapDelta = delta;
        if (delta > 0) stat.heapIncrease += delta; else stat.heapDecrease -= delta;
    }
}
function recordAsync(pluginName: string, surface: string, duration: number) {
    const stat = getPluginStats(pluginName);
    const surfaceStat = getSurfaceStats(stat, surface);
    stat.asyncCalls++; stat.asyncMs += duration; stat.lastSeen = Date.now(); surfaceStat.asyncMs += duration;
}
...
function normalizeCodeSnippet(code: string) { return code.trim().replace(/\n{3,}/g, "\n\n").slice(0, 6000); }
...
function rememberSourceSnippet(pluginName: string, surface: string, label: string, source: unknown) {
    ...
    if (snippets.some(s => s.surface === surface && s.code === code)) return;
...
function asRecord(value: unknown) { return isObject(value) ? value as Record<PropertyKey, unknown> : null; }
function rememberPatchSnippets(plugin: Plugin) {
    for (const [patchIndex, patch] of (plugin.patches ?? []).entries()) {
        ...
        for (const [replacementIndex, replacement] of replacements.entries()) {
            rememberSourceSnippet(plugin.name, "patch", `patch ${patchIndex + 1}.${replacementIndex + 1}`, [
                `find: ${stringifyCodePart(patch.find)}`, `match: ${stringifyCodePart(replacement.match)}`, `replace: ${stringifyCodePart(replacement.replace)}`
            ].join("\n"));
        }
    }
}
function instrumentPlugin(plugin: Plugin) {
    ...
    if (fluxRecord) for (const event of Object.keys(fluxRecord)) wrapObjectMethod(fluxRecord, event, plugin.name, `flux ${event}`);

    const contextMenuRecord = asRecord(plugin.contextMenus);
    if (contextMenuRecord) for (const menu of Object.keys(contextMenuRecord)) wrapObjectMethod(contextMenuRecord, menu, plugin.name, `context menu ${menu}`);

    const renderFields = [["chatBarButton", "render", "chat bar button"], ["headerBarButton", "render", "header bar button"], ["userAreaButton", "render", "user area button"]] as const;
    for (const [field, key, surface] of renderFields) {
        const owner = asRecord((pluginRecord as any)[field]);
...
function rememberResource(map: Map<number, string>, id: number, pluginName: string, key: ResourceKey) { map.set(id, pluginName); changeResource(pluginName, key, 1); }
...
function changeListenerCount(pluginName: string, delta: 1 | -1) {
    ...
    if (count === 0) listenerCountByPlugin.delete(pluginName); else listenerCountByPlugin.set(pluginName, count);
...
function rememberListener(target: EventTarget, type: string, listener: EventListenerOrEventListenerObject, pluginName: string, options?: boolean | AddEventListenerOptions) {
    ...
    if (!targetListeners) { targetListeners = new Map(); listenerOwners.set(target, targetListeners); }
...
function installGlobalProbes() {
    ...
    window.setTimeout = ((handler: string | ((...args: unknown[]) => void), timeout?: number, ...args: unknown[]) => {
        ...
        if (!context || typeof handler !== "function") return originalSetTimeout!(handler, timeout, ...args);
        let id = 0;
    ...
    window.setInterval = ((handler: string | ((...args: unknown[]) => void), timeout?: number, ...args: unknown[]) => {
        const context = currentContext();
        if (!context || typeof handler !== "function") return originalSetInterval!(handler, timeout, ...args);
        const wrapped = (...callbackArgs: unknown[]) => runMeasured(context.pluginName, "interval", () => handler(...callbackArgs));
...
function restoreGlobalProbes() {
    ...
    originalSetTimeout = undefined; originalClearTimeout = undefined; originalSetInterval = undefined;
    originalClearInterval = undefined; originalRequestAnimationFrame = undefined; originalCancelAnimationFrame = undefined;
    originalAddEventListener = undefined; originalRemoveEventListener = undefined;
}
function resetStats() {
    stats.clear();
    startedAt = Date.now();
...
function getHotSurface(stat: PluginStats) {
    const [surface] = Object.entries(stat.surfaces).sort(([, a], [, b]) => b.totalMs - a.totalMs)[0] ?? [];
...
function getImpact(row: PluginStats) {
    ...
    return row.totalMs + row.maxMs * 2 + row.slowCalls * settings.store.slowCallThresholdMs + row.asyncMs * 0.05
        + row.heapIncrease / 65536 + row.intervals * 120 + row.pendingTimeouts * 2 + row.animationFrames * 4 + row.listeners * 5 + resources;
}
function buildRows(showDisabled: boolean, showApiPlugins: boolean, sortBy: SortBy) {
    ...
    for (const [name, stat] of stats) {
        ...
        rows.set(name, { ...stat, enabled: plugin ? isPluginEnabled(name) : false, api, impact: getImpact(stat), hotSurface: getHotSurface(stat), resources: stat.intervals + stat.pendingTimeouts + stat.animationFrames + stat.listeners });
    }
    for (const plugin of Object.values(plugins)) {
        ...
        rows.set(plugin.name, { ...stat, enabled, api, impact: 0, hotSurface: "No samples", resources: 0 });
    }
    const sorted = Array.from(rows.values());
    sorted.sort((a, b) => {
        switch (sortBy) {
            case "cpu": return b.totalMs - a.totalMs;
            case "memory": return b.heapIncrease - a.heapIncrease;
            case "calls": return b.calls - a.calls;
            case "resources": return b.resources - a.resources;
            default: return b.impact - a.impact;
        }
    });
    return sorted;
}
function getMemory() { return (performance as PerformanceWithMemory).memory; }
function formatMs(value: number) { return value >= 1000 ? `${(value / 1000).toFixed(2)}s` : `${value.toFixed(value >= 10 ? 0 : 1)}ms`; }
...
function matchesPluginSearch(pluginName: string, query: string) { return !query || pluginName.toLowerCase().includes(query); }
...
function getImpactReasons(row: DiagnosticsRow): ImpactReason[] {
    ...
    if (row.totalMs >= 1000) reasons.push({ label: "High CPU", value: formatMs(row.totalMs), description: "Measured callbacks spent a lot of time on the JavaScript thread.", level: "high" });
    else if (row.totalMs >= 200) reasons.push({ label: "Noticeable CPU", value: formatMs(row.totalMs), description: "This plugin consumed enough callback time to be worth checking.", level: "medium" });

    if (row.maxMs >= 50) reasons.push({ label: "Slow spike", value: formatMs(row.maxMs), description: "A single call was long enough to cause a visible stutter.", level: "high" });
    else if (row.maxMs >= settings.store.slowCallThresholdMs) reasons.push({ label: "Spike over threshold", value: formatMs(row.maxMs), description: "At least one call exceeded the configured slow call threshold.", level: "medium" });

    if (row.slowCalls >= 10) reasons.push({ label: "Many slow calls", value: formatNumber(row.slowCalls), description: "The issue appears repeatedly.", level: "high" });
    else if (row.slowCalls > 0) reasons.push({ label: "Slow calls", value: formatNumber(row.slowCalls), description: "Some calls were slower than the configured threshold.", level: "medium" });

    if (row.heapIncrease >= 50 * 1024 * 1024) reasons.push({ label: "Growing heap", value: formatBytes(row.heapIncrease), description: "Many observed JavaScript allocations.", level: "high" });
    else if (row.heapIncrease >= 10 * 1024 * 1024) reasons.push({ label: "Noticeable memory", value: formatBytes(row.heapIncrease), description: "The heap delta is large enough to contribute to memory pressure.", level: "medium" });

    if (row.intervals > 0) reasons.push({ label: "Active intervals", value: formatNumber(row.intervals), description: "Intervals keep running code while the plugin is active.", level: row.intervals >= 3 ? "high" : "medium" });

    if (row.listeners >= 20) reasons.push({ label: "Many listeners", value: formatNumber(row.listeners), description: "Many listeners can increase work on frequent events.", level: "high" });
    else if (row.listeners > 0) reasons.push({ label: "Active listeners", value: formatNumber(row.listeners), description: "This plugin still has runtime listeners registered.", level: "low" });

    if (row.animationFrames > 0) reasons.push({ label: "Frame callbacks", value: formatNumber(row.animationFrames), description: "Frame-bound callbacks can directly affect client smoothness.", level: "medium" });
    if (row.pendingTimeouts >= 10) reasons.push({ label: "Pending timeouts", value: formatNumber(row.pendingTimeouts), description: "Many scheduled timeouts can create bursts of work later.", level: "medium" });

    if (reasons.length === 0 && row.impact > 0) reasons.push({ label: "Low impact", value: row.impact.toFixed(0), description: "This plugin has samples, but no single signal is very high.", level: "low" });
    return reasons;
}
function getRecommendation(row: DiagnosticsRow, reasons: ImpactReason[]) {
    const highReasons = reasons.filter(r => r.level === "high").length;
    if (row.impact >= 2000 || highReasons >= 2) return { shouldDisable: true, text: "Temporarily disabling this plugin is recommended so you can compare client smoothness." };
    if (row.impact >= 500 || highReasons === 1) return { shouldDisable: false, text: "Worth watching. Disable it only if you notice lag during related actions." };
    return { shouldDisable: false, text: "Disabling it does not seem necessary right now." };
}
...
function buildImpactAnalysis(rows: DiagnosticsRow[]) {
    return rows.filter(row => row.enabled && !row.api && row.impact > 0).map(row => {
        const reasons = getImpactReasons(row);
        const recommendation = getRecommendation(row, reasons);
        return { row, reasons, recommendation: recommendation.text, shouldDisable: recommendation.shouldDisable, summary: reasons[0]?.description ?? "Runtime samples have been collected for this plugin.", snippets: getRelevantSnippets(row) };
    }).sort((a, b) => b.row.impact - a.row.impact).slice(0, 20);
}
function getReportRows() { return buildRows(false, false, "impact").slice(0, 25); }
function copyReport() {
    const memory = getMemory();
    const lines = [
        "Client diagnostics", `Collected for ${formatMs(Date.now() - startedAt)}`, `Heap used: ${formatBytes(memory?.usedJSHeapSize)}`, "",
        "Plugin | Impact | CPU | Calls | Slow | Heap + | Resources | Hot surface",
        ...getReportRows().map(row => [row.name, row.impact.toFixed(0), formatMs(row.totalMs), formatNumber(row.calls), formatNumber(row.slowCalls), formatBytes(row.heapIncrease), formatNumber(row.resources), row.hotSurface].join(" | "))
    ];
...
function copyImpactAnalysisReport() {
    ...
    const lines = [
        "Impact analysis", `Collected for ${formatMs(Date.now() - startedAt)}`, "", "Plugin | Impact | Recommendation | Signals | Hot surface",
        ...items.map(item => [item.row.name, item.row.impact.toFixed(0), item.recommendation, item.reasons.map(r => `${r.label}: ${r.value}`).join(", ") || "No signals", item.row.hotSurface].join(" | "))
    ];
    copyWithToast(lines.join("\n"));
}
function copyPluginMonitorReport(selectedRow: DiagnosticsRow, rows: DiagnosticsRow[]) {
    ...
    const surfaces = Object.entries(selectedRow.surfaces).sort(([, a], [, b]) => b.totalMs - a.totalMs).slice(0, 6);
    const lines = [
        "Plugin monitor", `Collected for ${formatMs(Date.now() - startedAt)}`, `Plugin: ${selectedRow.name}`,
        `Status: ${selectedRow.enabled ? "Enabled" : "Disabled"}${selectedRow.api ? " API" : ""}`, "", "Metric | Value",
        `Impact | ${selectedRow.impact.toFixed(0)}`, `Extra CPU | ${formatPercent(getPercent(selectedRow.totalMs, elapsedMs))}`,
        ...
        `Observed heap + | ${formatBytes(selectedRow.heapIncrease)}`, `Max call | ${formatMs(selectedRow.maxMs)}`,
        `Slow calls | ${formatNumber(selectedRow.slowCalls)}`, `Resources | ${formatNumber(selectedRow.resources)}`, "", "Signals",
        ...(reasons.length > 0 ? reasons.map(r => `${r.label}: ${r.value}. ${r.description}`) : ["No risk signal for this plugin yet."]), "",
        "Most expensive surfaces",
        ...(surfaces.length > 0 ? surfaces.map(([surface, stat]) => `${surface} | ${formatMs(stat.totalMs)} | ${formatNumber(stat.calls)} calls | ${formatMs(stat.maxMs)} max`) : ["No measured surface yet."]),
        "", "Top measured plugins", "Plugin | Impact | CPU share",
        ...measuredRows.slice(0, 16).map(row => `${row.name} | ${row.impact.toFixed(0)} | ${formatPercent(getPercent(row.totalMs, totalCpu))}`)
    ];
    copyWithToast(lines.join("\n"));
}
    ...
    return (
        <div className={cl("search-controls")}>
            <div className={cl("search-input")}>
                <TextInput placeholder="Search for a plugin..." value={query} onChange={onChange} />
            </div>
            <Button variant="secondary" onClick={onSearch} disabled={disabled}>
                <MagnifyingGlassIcon height={16} width={16} /> Search
            </Button>
        </div>
    );
}
...
function SurfaceBreakdown({ row }: { row: DiagnosticsRow; }) {
    const surfaces = Object.entries(row.surfaces).sort(([, a], [, b]) => b.totalMs - a.totalMs).slice(0, 6);
    if (surfaces.length === 0) return <BaseText size="sm" color="text-muted">No measured surface yet.</BaseText>;
    return (
...
function SnippetList({ item }: { item: ImpactAnalysisItem; }) {
    if (item.snippets.length === 0) {
        return <BaseText size="sm" color="text-muted">No snippet available — happens when the cost comes from already-patched code or functions that can't be inspected at runtime.</BaseText>;
    }
    return (
        <div className={cl("snippet-list")}>
            {item.snippets.map((snippet, index) => (
                <div className={cl("snippet")} key={`${snippet.surface}-${index}`}>
                    ...
                    <div className={cl("snippet-code")}><CodeBlock content={snippet.code} lang="tsx" /></div>
                </div>
            ))}
        </div>
    );
        ...
        <div className={cl("analysis-card", selected ? "selected" : "")}>
            <div className={cl("analysis-card-head")}>
                ...
                <div className={cl("analysis-side")}>
                    <BaseText className={cl("impact", impactClass)} size="sm" weight="semibold" tabularNumbers>{row.impact.toFixed(0)}</BaseText>
                    <Button variant={selected ? "primary" : "secondary"} size="small" onClick={onSelect}>{selected ? "Hide" : "Details"}</Button>
                </div>
            </div>
            <div className={cl("analysis-meta")}>
                <BaseText size="sm" color={item.shouldDisable ? "text-feedback-warning" : "text-muted"}>{item.recommendation}</BaseText>
...
function ImpactAnalysisPage() {
    ...
    const items = React.useMemo(() => buildImpactAnalysis(buildRows(false, false, "impact")), [elapsed]);
    const filteredItems = React.useMemo(() => items.filter(item => matchesPluginSearch(item.row.name, searchQuery)), [items, searchQuery]);
        ...
        <div className={cl("page")}>
            <div className={cl("toolbar")}>
                <div>
                    ...
                    <BaseText size="sm" color="text-muted">This page interprets the metrics and flags plugins that may make Abyss lag.</BaseText>
                </div>
                <div className={cl("actions")}>
                    <Button variant="secondary" onClick={copyImpactAnalysisReport} disabled={items.length === 0}><CopyIcon height={16} width={16} /> Copy report</Button>
                </div>
            </div>
            <SearchControls query={query} onChange={setQuery} onSearch={() => setSearchQuery(normalizeSearch(query))} disabled={items.length === 0} />
            {items.length === 0 ? (
                <div className={cl("empty")}>
                    <BaseText size="md" weight="semibold">No suspicious plugins yet.</BaseText>
                    <BaseText size="sm" color="text-muted">Use the client for a few minutes, then come back here.</BaseText>
                </div>
                ...
                <div className={cl("analysis-list")}>
                    {filteredItems.map(item => (
                        <ImpactAnalysisCard key={item.row.name} item={item} selected={selectedPlugin === item.row.name} onSelect={() => setSelectedPlugin(selectedPlugin === item.row.name ? undefined : item.row.name)} />
                    ))}
                </div>
            )}
        </div>
...
function PluginMonitorPage() {
    ...
    const elapsed = useFixedTimer({ interval: refreshMs });
    const memory = getMemory();
    const rows = React.useMemo(() => buildRows(showDisabled, showApiPlugins, "impact"), [elapsed, showDisabled, showApiPlugins]);
    const normalizedQuery = query.trim().toLowerCase();
    const filteredRows = React.useMemo(() => (normalizedQuery ? rows.filter(row => row.name.toLowerCase().includes(normalizedQuery)) : rows).slice(0, 16), [normalizedQuery, rows]);
    const measuredRows = rows.filter(row => row.calls > 0);
    ...
    function selectSearchResult() {
        const exactMatch = normalizedQuery ? filteredRows.find(row => row.name.toLowerCase() === normalizedQuery) ?? filteredRows.find(row => row.name.toLowerCase().startsWith(normalizedQuery)) : undefined;
        ...
        <div className={cl("page")}>
            <div className={cl("toolbar")}>
                <div>
                    ...
                    <BaseText size="sm" color="text-muted">Search a plugin and compare its measured CPU and RAM contribution.</BaseText>
                </div>
                <div className={cl("actions")}>
                    <Button variant="secondary" onClick={() => selectedRow && copyPluginMonitorReport(selectedRow, rows)} disabled={!selectedRow}><CopyIcon height={16} width={16} /> Copy report</Button>
                </div>
            </div>
            <SearchControls query={query} onChange={setQuery} onSearch={selectSearchResult} disabled={filteredRows.length === 0} />
                ...
                <div className={cl("monitor-results")}>
                    {filteredRows.length === 0 ? (
                        ...
                        <div className={cl("monitor-result", selectedRow?.name === row.name ? "selected" : "")} key={row.name}>
                            ...
                            <Button variant={selectedRow?.name === row.name ? "primary" : "secondary"} size="small" onClick={() => setSelectedPlugin(row.name)}>View</Button>
                        </div>
                    ))}
                </div>
                {selectedRow ? (
                    <div className={cl("monitor-panel")}>
                        <div className={cl("monitor-panel-head")}>
                            <div className={cl("analysis-title")}>
                                ...
                                <BaseText size="sm" color="text-muted" lineClamp={2}>{selectedRow.calls > 0 ? `Measured through ${formatNumber(selectedRow.calls)} callbacks.` : "No measured callbacks yet."}</BaseText>
                            </div>
                        ...
                        <div className={cl("detail-section")}>
                            ...
                            {reasons.length === 0 ? <BaseText size="sm" color="text-muted">No risk signal for this plugin yet.</BaseText> : (
                                <div className={cl("reason-list")}>
                                    {reasons.map(reason => (
                                        <div className={cl("reason-line")} key={`${reason.label}-${reason.value}`}>
                                            <ReasonPill reason={reason} />
...
function DiagnosticsPage() {
    ...
    const elapsed = useFixedTimer({ interval: refreshMs });
    const memory = getMemory();
    const rows = React.useMemo(() => buildRows(showDisabled, showApiPlugins, sortBy as SortBy), [elapsed, showDisabled, showApiPlugins, sortBy]);
    const filteredRows = React.useMemo(() => rows.filter(row => matchesPluginSearch(row.name, searchQuery)), [rows, searchQuery]);
        ...
        <div className={cl("page")}>
            <div className={cl("toolbar")}>
                <div>
                    ...
                    <BaseText size="sm" color="text-muted">CPU is measured as JavaScript callback time. Memory uses Chromium heap deltas when available.</BaseText>
                </div>
                <div className={cl("actions")}>
                    <Button variant="secondary" onClick={copyReport}><CopyIcon height={16} width={16} /> Copy report</Button>
                    <Button variant="secondary" onClick={resetStats}><ResetIcon height={16} width={16} /> Reset</Button>
                </div>
            </div>
            <div className={cl("metrics")}>
            ...
            <SearchControls query={query} onChange={setQuery} onSearch={() => setSearchQuery(normalizeSearch(query))} disabled={rows.length === 0} />
        ...
        <div className={cl("page")}>
            <div className={cl("toolbar")}>
                <div>
                    ...
                    <BaseText size="sm" color="text-muted">What Client Diagnostics measures and how to read the numbers.</BaseText>
                </div>
            </div>
            <div className={cl("guide-section")}>
                <BaseText size="md" weight="semibold">What it is for</BaseText>
                <BaseText size="sm" color="text-muted">Client Diagnostics helps find plugins that may cause lag, freezes, or extra memory usage by measuring time spent inside plugin functions, JavaScript memory growth, and resources that remain active.</BaseText>
                <BaseText size="sm" color="text-muted">These are samples collected while you use Abyss. A plugin not used during collection may look light even if it does more work in other situations.</BaseText>
            </div>
            <div className={cl("guide-section")}>
            ...
            <div className={cl("guide-section")}>
                ...
                <BaseText size="sm" color="text-muted">CPU and RAM aren't per-plugin values read directly from the OS — this estimates cost by observing JavaScript work, Chromium heap data, and resources created during plugin callbacks.</BaseText>
            </div>
        </div>
    );
}
function ClientDiagnosticsPage() {
    ...
    return (
        <div className={cl("root")}>
            <TabBar type="top" look="brand" selectedItem={tab} onItemSelect={setTab} className={cl("tabs")}>
...
export default definePlugin({
    ...
    start() {
        try {
            ...
            SettingsPlugin.customEntries.push({ key: ENTRY_KEY, title: "Client Diagnostics", Component: DiagnosticsPageWrapped, Icon: ClockIcon });
        } catch (error) {
