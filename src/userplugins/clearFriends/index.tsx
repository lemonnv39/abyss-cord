...
// Discord's own official endpoint — more reliable than hunting for a
// webpack-internal action, since that search can (and has) landed on the
// wrong module. This is the exact same endpoint Discord's own client hits
// when you click "Remove Friend" in the UI.
function removeFriend(id: string): Promise<any> {
    return RestAPI.del({ url: `/users/@me/relationships/${id}` });
}
...
function SpinnerIcon() {
    return (
        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" style={{ animation: "cfr-spin 1s linear infinite" }}>
...
function ClearFriendsModal({ rootProps }: { rootProps: any; }) {
    ...
    async function removeSelected() {
        ...
        for (let i = 0; i < selected.length; i++) {
            ...
            try {
                ...
                console.warn("[ClearFriends] Failed:", f.id, e);
        ...
        <ModalRoot {...rootProps} size="medium">
            <ModalHeader separator={false}>
                <Forms.FormTitle tag="h4" style={{ margin: 0, display: "flex", alignItems: "center", gap: 8, color: "#ffffff" }}>
                    <BulkRemoveIcon width={16} height={16} /> Remove friends
                </Forms.FormTitle>
                <ModalCloseButton onClick={rootProps.onClose} />
            </ModalHeader>
            <ModalContent className="cfr-content">
                <div className="cfr-search-bar">
                    ...
                    <input className="cfr-search-input" placeholder="Search for a friend..." value={search} onChange={e => setSearch(e.target.value)} autoFocus />
                    {search && (
                        <button className="cfr-search-clear" onClick={() => setSearch("")}>
                    ...
                    <div className="cfr-search-sep" />
                    <button className="cfr-select-all" onClick={toggleAll} title={allSelected ? "Deselect all" : "Select all"}>
                ...
                <Forms.FormTitle tag="h5" style={{ margin: "4px 0" }}>
                    ...
                    {selected.length > 0 && <span className="cfr-selected-count"> · {selected.length} selected</span>}
                </Forms.FormTitle>

                <div className="cfr-list">
                    {filtered.length === 0 && <div className="cfr-empty">No friends found</div>}
                    {filtered.map(f => (
                        <div key={f.id} className={`cfr-row ${f.selected ? "cfr-row--selected" : ""}`} onClick={() => !f.removing && toggle(f.id)}>
                            <div className="cfr-checkbox"><CheckboxIcon checked={f.selected} /></div>
                            <img src={getAvatarUrl(f.id, f.avatar)} className="cfr-avatar" alt="" />
                            <div className="cfr-info">
                                <span className="cfr-name">{f.globalName}</span>
                                <span className="cfr-tag">@{f.username}</span>
                            </div>
                ...
                {removing && (
                    <div className="cfr-progress-wrap">
                        <div className="cfr-progress-bar"><div className="cfr-progress-fill" style={{ width: `${Math.round(progress.done / progress.total * 100)}%` }} /></div>
                        <span className="cfr-progress-label">{progress.done}/{progress.total}</span>
                    </div>
                )}
            </ModalContent>
            <ModalFooter>
                <button className="cfr-btn cfr-btn-secondary" onClick={() => setFriends(loadFriends())} disabled={removing}>
                    <RefreshIcon /> Refresh
                </button>
                <button className="cfr-btn cfr-btn-danger" onClick={removeSelected} disabled={!selected.length || removing} style={{ flex: 1 }}>
                    {removing ? <><SpinnerIcon /> {progress.done}/{progress.total}</> : <><TrashIcon /> Delete ({selected.length})</>}
                </button>
            </ModalFooter>
        </ModalRoot>
    );
...
function ClearFriendsButton() {
    return (
        <HeaderBarButton
            icon={BulkRemoveIcon}
            tooltip="Friends deletion"
            onClick={() => openModal(props => <ClearFriendsModal rootProps={props} />)}
        />
    );
}
...
export default definePlugin({
    ...
    description: "Delete multiple friends at once, from a searchable checklist in the header bar.",
    authors: [{ name: "0ctane", id: 0n }],
    headerBarButton: { icon: BulkRemoveIcon, render: ClearFriendsButton, priority: 5 },
});
